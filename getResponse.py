...
def Histogram_Response_asy(branchString, labelName, errbars_shift, ScaleErr, rangemin, rangemax):
    #DFName.loc[DFName[Target_Pt]]
    Response = -(DFName[branchString][(DFName[Target_Pt]>rangemin) & (DFName[Target_Pt]<rangemax)])/DFName[Target_Pt][(DFName[Target_Pt]>rangemin) & (DFName[Target_Pt]<rangemax)]
    Response = Response[~np.isnan(Response)]
    Response2 = -(Response-1)+1
    Mean = np.mean(Response)
    Std = np.std(Response)

    plt.hist(Response, bins=nbinsHist, range=[ResponseMin, ResponseMax], label=labelName+', %8.2f $\pm$ %8.2f'%(Mean, Std), histtype='step', ec=colors_InOut[errbars_shift])
    plt.hist(Response2, bins=nbinsHist, range=[ResponseMin, ResponseMax], label=labelName+' mirrored, %8.2f $\pm$ %8.2f'%(Mean, Std), histtype='step', ec=colors_InOut[errbars_shift])
...
def Histogram_Response_asy2(branchString, labelName, errbars_shift, ScaleErr, rangemin, rangemax):
    ...
    #sns_plot = sns.heatmap(0.5*(_[1:]+_[:-1]), n-nm, annot=False, ax=ax, cmap="YlGnBu", linewidths=.5, cbar_kws={"orientation": "vertical"})



    #plt.hist(Response-Response2, bins=nbinsHist, range=[-2, 2], label=labelName+', %8.2f $^{+ %8.2f}_{- %8.2f}$'%(Mean,Stdup, Stddown), histtype='step', ec=colors_InOut[errbars_shift])
    #plt.hist(Response2, bins=nbinsHist, range=[ResponseMin, ResponseMax], label=labelName+' mirrored, %8.2f $\pm$ %8.2f'%(Mean, Std), histtype='step', ec=colors_InOut[errbars_shift])
...
def Histogram_Response_filled(branchString, labelName, bin, ScaleErr):
    ...
    #plt.fill_between(0.5*(_[:-1]+_[1:]), 0, n, interpolate=False,
    #            where=((_[:-1])>=1), alpha=0.2)
    #plt.fill_between(0.5*(_[:-1]+_[1:]), 0, n, interpolate=False,
    #            where=((_[1:])<=1), alpha=0.2)
...
def getPlotsOutput(inputD, filesD, plotsD,DFName, DFName_nVertex, Target_Pt, Target_Phi):
    ...
    ax.legend(ncol=1, handles=handles, bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0., fontsize='x-small', title=LegendTitle, numpoints=1	)
    plt.grid()
    plt.ylim(ResponseMinErr+0.3, ResponseMaxErr+0.3)
    plt.savefig("%sResponse_PV_r.png"%(plotsD), bbox_inches="tight")
    plt.close()
    ...
    Histogram_Response_asy('recoilslimmedMETsPuppi_LongZ', 'Puppi', 4, ScaleErr, 20, 200)
    Histogram_Response_asy('NN_LongZ', 'NN', 6, ScaleErr, 20, 200)
    Histogram_Response_asy('recoilslimmedMETs_LongZ', 'PF', 1, ScaleErr, 20, 200)
    ...
    plt.grid()
    plt.xlim(ResponseMin, ResponseMax)
    plt.savefig("%sAsy_Hist_Response.png"%(plotsD), bbox_inches="tight")
    plt.close()
    ...
    sns.boxplot( y=y, x=x, palette=[colors_InOut[1], colors_InOut[4], colors_InOut[6]], showfliers=False)
    ...
    plt.savefig("%sAsy2_Hist_Response.png"%(plotsD), bbox_inches="tight")
    plt.close()
