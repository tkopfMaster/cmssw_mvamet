...
def plotMVAResolutionOverNVertex_woutError_para(branchString, labelName, errbars_shift, ScaleErr, rangemin, rangemax):
    ...
    mean = sy / n
    std = np.sqrt(sy2/n - mean*mean)
    meanc, stdc = np.mean(mean), np.std((-(DFName_nVertex[branchString])-DFName_nVertex[Target_Pt]))
...
def Hist_InvMET(branchString, labelName, errbars_shift, ScaleErr):
    ...
    plt.hist(Response, bins=nbinsHist, range=[ResponseMin, ResponseMax], label='$\\frac{1}{p_T^{recoil}}$, %8.2f $\pm$ %8.2f'%(Mean, Std), histtype='step', ec=colors_InOut[errbars_shift])
...
def Hist_Resolution_para(branchString, labelName, errbars_shift, ScaleErr, rangemin, rangemax):
    ...
    plt.hist((-(DFName[branchString][(DFName[Target_Pt]>rangemin) & (DFName[Target_Pt]<rangemax)])-DFName[Target_Pt][(DFName[Target_Pt]>rangemin) & (DFName[Target_Pt]<rangemax)]), bins=nbinsHist, range=[ResolutionParaMin, ResolutionParaMax], label=labelName, histtype='step', ec=colors_InOut[errbars_shift])
...
def Histogram_Response_NN(branchString, labelName, bin, ScaleErr):
    ...
    Reso = np.divide(-(DFName[branchString]),DFName[Target_Pt].values)
    n, _ = np.histogram(Reso[~np.isnan(Reso)], bins=nbinsHistBin, range=[ResponseMin, ResponseMax])
    plt.hist(Reso[~np.isnan(Reso)], bins=nbinsHistBin, range=[ResponseMin, ResponseMax], label=labelName+': %8.2f $\pm$ %8.2f'%(Mean, Std), histtype='step', ec=colors_InOut[bin], stacked=False, linewidth=3.0)
...
def Histogram_Resolution_para_ax(branchString, labelName, bin, ScaleErr, ax):
    Mean = np.mean((-(DFName[branchString])-DFName[Target_Pt]))
    Std = np.std((-(DFName[branchString])-DFName[Target_Pt]))
    ax.hist(-(DFName[branchString])-DFName[Target_Pt], bins=nbinsHist, range=[ResolutionParaMin, ResolutionParaMax], label=labelName+': %8.2f $\pm$ %8.2f'%(Mean, Std), histtype='step', ec=colors_InOut[bin])
...
def Histogram_Resolution_perp_ax(branchString, labelName, bin, ScaleErr, ax):
    Mean = np.mean(DFName[branchString])
    Std = np.std(DFName[branchString])
    ax.hist(DFName[branchString], bins=nbinsHist, range=[ResolutionPerpMin, ResolutionPerpMax], label=labelName+': %8.2f $\pm$ %8.2f'%(Mean, Std), histtype='step', ec=colors_InOut[bin])
...
def Histogram_Response_asy2(branchString, labelName, errbars_shift, ScaleErr, rangemin, rangemax):
    ...
    Response = -(DFName[branchString][(DFName[Target_Pt]>rangemin) & (DFName[Target_Pt]<rangemax)])/DFName[Target_Pt][(DFName[Target_Pt]>rangemin) & (DFName[Target_Pt]<rangemax)]
    Response = Response[~np.isnan(Response)]
    print(str(Histogram_Response_asy2), branchString, "left quartile 25 percent", np.percentile(Response,50)-np.percentile(Response,50-25))
    print(str(Histogram_Response_asy2), branchString, "right quartile 25 percent", np.percentile(Response,50+25)-np.percentile(Response,50))
    print(str(Histogram_Response_asy2), branchString, "median", np.percentile(Response,50))
    return Response

def Histogram_Resolution_para_asy2(branchString, labelName, errbars_shift, ScaleErr, rangemin, rangemax):
    #DFName.loc[DFName[Target_Pt]]
    Res = (-(DFName[branchString])-DFName[Target_Pt])
    print(str(Histogram_Resolution_para_asy2), branchString, "left quartile 25 percent", np.percentile(Res,50)-np.percentile(Res,50-25))
    print(str(Histogram_Resolution_para_asy2), branchString, "right quartile 25 percent", np.percentile(Res,50+25)-np.percentile(Res,50))
    print(str(Histogram_Resolution_para_asy2), branchString, "median", np.percentile(Res,50))
    return Res
...
def Histogram_Resolution_perp_asy2(branchString, labelName, errbars_shift, ScaleErr, rangemin, rangemax):
    ...
    print(str(Histogram_Resolution_perp_asy2), branchString, "left quartile 25 percent", np.percentile(Res,50)-np.percentile(Res,50-25))
    print(str(Histogram_Resolution_perp_asy2), branchString, "right quartile 25 percent", np.percentile(Res,50+25)-np.percentile(Res,50))
    print(str(Histogram_Resolution_perp_asy2), branchString, "median", np.percentile(Res,50))
    return Res
...
def Hist_PerpZ(branchString, labelName, errbars_shift, ScaleErr):
    ...
    Std = np.std((DFName_nVertex[branchString]))
    if branchString in ['NN_PerpZ', 'recoilslimmedMETs_PerpZ']:
        plt.hist(((DFName_nVertex[branchString])), bins=nbinsHist, range=[-60, 60], label=labelName+': %8.2f $\pm$ %8.2f'%(Mean, Std), histtype='step', ec=colors_InOut[errbars_shift], linewidth=1.5)
    else:
        plt.hist(((DFName_nVertex[branchString])), bins=nbinsHist, range=[-60, 60], label=labelName+': %8.2f $\pm$ %8.2f'%(Mean, Std), histtype='step', ec=colors_InOut[errbars_shift])
...
def getPlotsOutput(inputD, filesD, plotsD,DFName, DFName_nVertex, Target_Pt, Target_Phi):
    ...
    NPlotsLines = 6
    MVA_NPlotsLines = 3
    pTRangeString_Err = '$%8.2f\ \mathrm{GeV} < p_T^{\ recoil} \leq %8.2f\ \mathrm{GeV}$ \n $\mathrm{\# Vertex} \leq 50$ \\ Error scaled: '%(pTMin,pTMax)+str(ScaleErr)
    pTRangeString= '$%8.2f\ \mathrm{GeV} < p_T^{\ recoil} \leq %8.2f\ \mathrm{GeV}$ \n $\mathrm{\# Vertex} \leq 50$'%(pTMin,pTMax)
    pTRangeString_low= pTRangeString_mid= pTRangeString_high= pTRangeString



    pTRangeString_Tresh = '$1\ \mathrm{GeV} < p_T^{\ recoil} \leq 200\ \mathrm{GeV}$ \n $\mathrm{\# Vertex} \leq 50$'
    ...
    plt.xlabel('$p_T^{\ recoil} $ in GeV', fontsize=18)
    plt.ylabel('$\\langle \\frac{U_{\parallel}}{p_T^{\ recoil}} \\rangle$ ', fontsize=18)
    #plt.title('Response $U_{\parallel}$', fontsize=18)

    legend = plt.legend(ncol=1, handles=handles, bbox_to_anchor=(0.95, 0.05), loc=4, borderaxespad=0., fontsize='large', title='MET definition', numpoints=1, framealpha=1.0	)
    ...
    plt.xlabel('$p_T^{\ recoil} $ in GeV', fontsize=18)
    plt.ylabel('$\sigma \\left( U_{\parallel} - p_T^{recoil} \\right) $ in GeV', fontsize=18)
    #plt.ylabel('$\sigma \\left( \\frac{u_{\perp}}{p_T^{\ recoil}} \\right) $ in GeV')
    #plt.title('Resolution $U_{\perp}$', fontsize=18)

    legend = plt.legend(ncol=1, handles=handles, bbox_to_anchor=(0.95, 0.05), loc=4, borderaxespad=0., fontsize='x-large', title='MET definition', numpoints=1, framealpha=1.0	)
    #legend = plt.legend(ncol=1, handles=handles, bbox_to_anchor=(0.95, 0.05), loc=4, borderaxespad=0., fontsize='large', title='MET definition', numpoints=1, framealpha=1.0	)
    plt.setp(legend.get_title(),fontsize='x-large')
    plt.tick_params(axis='both', which='major', labelsize=18)
    plt.grid(color='black', linestyle='-', linewidth=0.1)
    ...
    ax.set_position([box.x0, box.y0, box.width * 0.85, box.height])
    handles, labels = ax.get_legend_handles_labels()
    #handles.insert(0,mpatches.Patch(color='none', label='$%8.2f\ \mathrm{GeV} < p_T^{\ recoil} \leq %8.2f\ \mathrm{GeV}$ \n $\mathrm{\# Vertex} \leq 50$'%(20,40)))

    plt.xlabel('$U_{\parallel} - p_T^{recoil}$ in GeV', fontsize=18)
    plt.ylabel('Counts', fontsize=18)
    #plt.ylabel('$\sigma \\left( \\frac{u_{\parallel}}{p_T^{\ recoil}} \\right) $ in GeV')
    #plt.title('Resolution $U_{\parallel}$', fontsize=18)

    legend = plt.legend(ncol=1, bbox_to_anchor=(0.95, 1.05), loc=4, borderaxespad=0., fontsize='large', title='MET definition', numpoints=1, framealpha=1.0, handles=[Line2D([], [], c=h.get_edgecolor()) for h in handles],  labels=labels)
    plt.setp(legend.get_title(),fontsize='x-large')
    plt.tick_params(axis='both', which='major', labelsize=18)
    plt.grid(color='black', linestyle='-', linewidth=0.1)
    ...
    legend = plt.legend(ncol=1, handles=handles, bbox_to_anchor=(0.95, 0.05), loc=4, borderaxespad=0., fontsize='x-large', title='MET definition', numpoints=1, framealpha=1.0	)
    #legend = plt.legend(ncol=1, handles=handles, bbox_to_anchor=(0.95, 0.05), loc=4, borderaxespad=0., fontsize='large', title=LegendTitleStd, numpoints=1, framealpha=1.0	)
    plt.setp(legend.get_title(),fontsize='x-large')
    plt.tick_params(axis='both', which='major', labelsize=18)
    plt.grid(color='black', linestyle='-', linewidth=0.1)
    plt.ylim(ylimResMVAMin, ylimResMVAMax)
    ...
    legend = plt.legend(ncol=1, handles=handles, bbox_to_anchor=(0.95, 0.05), loc=4, borderaxespad=0., fontsize='x-large', title='MET definition', numpoints=1, framealpha=1.0	)
    #legend = plt.legend(ncol=1, handles=handles, bbox_to_anchor=(0.95, 0.05), loc=4, borderaxespad=0., fontsize='large', title=LegendTitle, numpoints=1, framealpha=1.0	)
    plt.setp(legend.get_title(),fontsize='x-large')
    plt.tick_params(axis='both', which='major', labelsize=18)
    plt.grid(color='black', linestyle='-', linewidth=0.1)
    plt.ylim(ylimResMVAMin, ylimResMVAMax)
    ...
    legend = plt.legend(ncol=1, bbox_to_anchor=(0.95, 1.05), loc=4, borderaxespad=0., fontsize='large', title=LegendTitle, numpoints=1	, handles=[Line2D([], [], c=h.get_edgecolor()) for h in handles],  labels=labels	)
    plt.setp(legend.get_title(),fontsize='x-large')
    plt.tick_params(axis='both', which='major', labelsize=18)
    plt.tick_params(axis='both', which='major', labelsize=18)
    plt.grid(color='black', linestyle='-', linewidth=0.1)
    ...
    legend = plt.legend(ncol=1, bbox_to_anchor=(0.95, 1.05), loc=4, borderaxespad=0., fontsize='large', title=LegendTitle, numpoints=1, framealpha=1.0	, handles=[Line2D([], [], c=h.get_edgecolor()) for h in handles],  labels=labels	)
    plt.setp(legend.get_title(),fontsize='x-large')
    plt.tick_params(axis='both', which='major', labelsize=18)
    plt.grid(color='black', linestyle='-', linewidth=0.1)
    ...
    legend = ax1.legend(ncol=1, bbox_to_anchor=(0.95, 1.05), loc=4, borderaxespad=0., fontsize='large', title=LegendTitle, numpoints=1	, handles=[Line2D([], [], c=h.get_edgecolor()) for h in handles],  labels=labels	)
    plt.setp(legend.get_title(),fontsize='x-large')
    ...
    usermedians = [np.mean(Histogram_Response_asy2('NN_LongZ', 'NN', 6, ScaleErr, 20, 200)), np.mean(Histogram_Response_asy2('recoilslimmedMETs_LongZ', 'PF', 1, ScaleErr, 20, 200)), np.mean(Histogram_Response_asy2('recoilslimmedMETsPuppi_LongZ', 'Puppi', 4, ScaleErr, 20, 200))]
    ...
    plt.xlabel('$  \\frac{U_{\parallel}}{p_T^{\ recoil}}} $')
    plt.savefig("%sHist_Response_wBoxplots_mean.png"%(plotsD), bbox_inches="tight")
    plt.close()
    ...
    df = pd.DataFrame(x, index=y)
    medianprops = dict(linestyle='-', linewidth=2.5, color='white')
    bplot1 = ax2.boxplot(x, labels=y, vert=False, patch_artist=True, showfliers=False,  medianprops=medianprops, showmeans=False)
    plt.yticks([1, 2, 3], y, size="x-large")
    #bplot1.tick_params(axis='both', which='minor', labelsize=18)
    colors = [colors_InOut[6], colors_InOut[1],colors_InOut[4]]
    for patch, color in zip(bplot1['boxes'], colors):
    ...
    plt.xlim(ResponseMin, ResponseMax)
    plt.xlabel('$  \\frac{U_{\parallel}}{p_T^{\ recoil}}} $')
    plt.savefig("%sHist_Response_wBoxplots_median.png"%(plotsD), bbox_inches="tight")
    plt.close()
    ...
    Histogram_Resolution_para_ax('recoilslimmedMETsPuppi_LongZ', 'Puppi', 4, ScaleErr, ax1)
    Histogram_Resolution_para_ax('recoilslimmedMETs_LongZ', 'PF', 1, ScaleErr, ax1)
    ...
    x = [Histogram_Resolution_para_asy2('NN_LongZ', 'NN', 6, ScaleErr, 20, 200),
        Histogram_Resolution_para_asy2('recoilslimmedMETs_LongZ', 'PF', 1, ScaleErr, 20, 200),
        Histogram_Resolution_para_asy2('recoilslimmedMETsPuppi_LongZ', 'Puppi', 4, ScaleErr, 20, 200)]
    df = pd.DataFrame(x, index=y)
    medianprops = dict(linestyle='-', linewidth=2.5, color='white')
    usermedians = [np.mean(Histogram_Resolution_para_asy2('NN_LongZ', 'NN', 6, ScaleErr, 20, 200)),np.mean(Histogram_Resolution_para_asy2('recoilslimmedMETs_LongZ', 'PF', 1, ScaleErr, 20, 200)),np.mean(Histogram_Resolution_para_asy2('recoilslimmedMETsPuppi_LongZ', 'Puppi', 4, ScaleErr, 20, 200))]
    bplot1 = ax2.boxplot(x, labels=y, vert=False, patch_artist=True, showfliers=False,  medianprops=medianprops, showmeans=False, usermedians=usermedians)
    plt.yticks([1, 2, 3], y, size="x-large")
    #bplot1.tick_params(axis='both', which='minor', labelsize=18)
    colors = [colors_InOut[6], colors_InOut[1],colors_InOut[4]]
    ...
    plt.savefig("%sHist_Resolution_para_wBoxplots.png"%(plotsD), bbox_inches="tight")
    plt.close()
    ...
    Histogram_Resolution_perp_ax('recoilslimmedMETsPuppi_PerpZ', 'Puppi', 4, ScaleErr, ax1)
    Histogram_Resolution_perp_ax('recoilslimmedMETs_PerpZ', 'PF', 1, ScaleErr, ax1)
    ...
    x = [Histogram_Resolution_perp_asy2('NN_PerpZ', 'NN', 6, ScaleErr, 20, 200),
        Histogram_Resolution_perp_asy2('recoilslimmedMETs_PerpZ', 'PF', 1, ScaleErr, 20, 200),
        Histogram_Resolution_perp_asy2('recoilslimmedMETsPuppi_PerpZ', 'Puppi', 4, ScaleErr, 20, 200)]
    df = pd.DataFrame(x, index=y)
    medianprops = dict(linestyle='-', linewidth=2.5, color='white')
    usermedians = [np.mean(Histogram_Resolution_perp_asy2('NN_PerpZ', 'NN', 6, ScaleErr, 20, 200)),np.mean(Histogram_Resolution_perp_asy2('recoilslimmedMETs_PerpZ', 'PF', 1, ScaleErr, 20, 200)),np.mean(Histogram_Resolution_perp_asy2('recoilslimmedMETsPuppi_PerpZ', 'Puppi', 4, ScaleErr, 20, 200))]
    bplot1 = ax2.boxplot(x, labels=y, vert=False, patch_artist=True, showfliers=False,  medianprops=medianprops, showmeans=False, usermedians=usermedians)
    plt.yticks([1, 2, 3], y, size="x-large")
    #bplot1.tick_params(axis='both', which='minor', labelsize=18)
    colors = [colors_InOut[6], colors_InOut[1],colors_InOut[4]]
    ...
    plt.savefig("%sHist_Resolution_perp_wBoxplots.png"%(plotsD), bbox_inches="tight")
    plt.close()
    ...
    legend = plt.legend(ncol=1, bbox_to_anchor=(1.05, 1.00), loc=4, borderaxespad=0., fontsize='large', title=LegendTitle, numpoints=1, framealpha=1.0, handles=[Line2D([], [], c=h.get_edgecolor()) for h in handles],  labels=labels)
    plt.setp(legend.get_title(),fontsize='x-large')
    plt.tick_params(axis='both', which='major', labelsize=18)
    plt.grid(color='black', linestyle='-', linewidth=0.1)
    plt.xlim(0, 70)
    ...
    plt.xlabel('$  \\frac{U_{\parallel}}{p_T^{\ recoil}}} $', fontsize=18)
    plt.ylabel('Counts', fontsize=18)
    ...
    legend = plt.legend(ncol=1, handles=handles, bbox_to_anchor=(1.05, 1.00), loc=4, borderaxespad=0., fontsize='large', title=LegendTitle, numpoints=1, framealpha=1.0	)
    plt.setp(legend.get_title(),fontsize='x-large')
    plt.tick_params(axis='both', which='major', labelsize=18)
    plt.grid(color='black', linestyle='-', linewidth=0.1)
    plt.xlim(ResponseMin, ResponseMax)
